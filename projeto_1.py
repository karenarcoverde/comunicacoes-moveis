...
with st.sidebar:
    ...
    R = st.number_input("Raio da célula R (km)", 0.1, 50.0, 1.0, 0.1)
    N = st.selectbox("Tamanho do cluster N", [3, 4, 7], index=2)
    S = st.number_input("Número de canais totais", 1, 2000, 395, 1)
    A = st.number_input("Tráfego oferecido (Erlangs)", 0.1, 5000.0, 100.0, 1.0)
...
# ── TAB 2: REM CCI com SIR e tabela antes/depois ────────────────────────
with tab2:
    cluster_cells = get_cluster_cells(N)
    cluster_set = set(cluster_cells)
    translations = get_cochannel(N)
    ...
    # grade de pontos para cálculo da SIR
    all_x = [v[0] for v in grid_centers.values()]
    all_y = [v[1] for v in grid_centers.values()]
    x_min = min(all_x)-R
    x_max = max(all_x)+R
    ...
    sc = ax2.scatter(rem_x,rem_y,c=rem_sir,cmap=cmap_sir,norm=norm_sir,s=5,marker="s",alpha=0.9)
    ...
    segundo_tick = valores_barra[1]
    st.pyplot(fig2,width="stretch")
    plt.close(fig2)

    # ── Tabela antes/depois ─────────────────────────────────────────────
    sir_min_db = segundo_tick
    if len(rem_sir) > 0:
        C_teorico = S // N
        fracao_critica = np.mean(rem_sir<sir_min_db)
        ...
        st.markdown("### Validação da capacidade com Erlang-B e SIR")
        ...
        tabela_capacidade = [
            {
                ...
                "Área crítica de SIR por célula":"Não considerada",
            ...
            },
            {
                ...
                "Área crítica de SIR por célula":f"{100*fracao_critica:.2f}%",
                "Probabilidade de bloqueio de Erlang-B":f"{100*B_efetivo:.2f}%"
            }
        ]

        st.table(tabela_capacidade)
        st.caption("A capacidade efetiva reduz o número de canais da célula de acordo com a fração da área em que a SIR fica abaixo do limiar mínimo adotado.")